...
public class AccountRepositoryMySQL implements AccountRepository{
    ...
    public List<Account> findAll() {
       /* List<Account> accounts = new ArrayList<>();
        ...
        try{  PreparedStatement statement=connection
                .prepareStatement( "Select * from repository_account");
    ...
    public Account findById(Long id) throws EntityNotFoundException {
      ...
      try{  PreparedStatement statement=connection
                .prepareStatement( "Select * from repository_account where id=" + id);
        ResultSet rs=statement.executeQuery();
        if (rs.next()) {
            return getAccountFromResultSet(rs);
        } else {
           ...
           /* Statement statement = connection.createStatement();
            ...
            statement.executeUpdate(sql);*/
             ...
             PreparedStatement removeStatement= connection
                    .prepareStatement("DELETE from repository_account where id >= 0" );
    ...
    public void transferMoney(Long idSend, Long idReceive, Double sum) {
        ...
        try {
            ...
            Account accountSend = findById(idSend);




            Account accountReceive=findById(idReceive);


            if(accountSend.getAmountOfMoney()>=sum)
            {
                update(accountSend.getAmountOfMoney()-sum, idSend);
                update(accountReceive.getAmountOfMoney()+sum, idReceive);
    ...
    public void processBill(Long id, Double payment)  {
        ...
        try {
            ...
            Account accountPayment=findById(id);

                if(accountPayment.getAmountOfMoney()>=payment)
                {
                    update(accountPayment.getAmountOfMoney()-payment, id);
